...
import 'package:melomix/view/splash_login.dart'; // Asegúrate de usar la ruta correcta
...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void _register() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final success = await _apiService.register(_email, _password);

        if (success) {
          // Navegar de vuelta a la pantalla de inicio de sesión si el registro es exitoso
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => SplashView()),
          );
        } else {
          _showAlert('Error', 'El registro falló. Inténtalo de nuevo.');
        }
      } catch (e) {
                  ...
                  child: Column(
                    children: [
                      ...
                      ElevatedButton(
                        ...
                        onPressed: _register,
                        child: _isLoading
                            ? CircularProgressIndicator() // Mostrar indicador de carga mientras se realiza el registro
                            : Text('Registrarse'),
                      ),
                      SizedBox(height: 10),
                      // Mensaje de error
                      ...
                      SizedBox(height: 10),
                      // Texto y botón para volver a iniciar sesión
                      Text(
